...
from utils.pt_utils import ensure_dir, link_file, load_model
from utils.transforms import normalize, pad_image_to_shape

LOGGER = logging.getLogger("dist_test")
...
class Evaluator(object):
    ...
    def worker(self, shred_list, device):
        ...
        LOGGER.info(
            "Load Model on Device %d: %.2fs" % (device, time.time() - start_load_time)
        )
    ...
    def whole_eval(self, img, output_size, input_size=None, device=None):
        ...
        if input_size is not None:
            pred = pred[
                :,
                margin[0] : (pred.shape[1] - margin[1]),
        ...
        if output_size is not None:
            pred = cv2.resize(
                pred, (output_size[1], output_size[0]), interpolation=cv2.INTER_LINEAR
            )
    ...
    def sliding_eval(self, img, crop_size, stride_rate, device=None):
        ...
        for s in self.multi_scales:
            img_scale = cv2.resize(
                img, None, fx=s, fy=s, interpolation=cv2.INTER_LINEAR
            )
            new_rows, new_cols, _ = img_scale.shape
            processed_pred += self.scale_process(
                img_scale, (ori_rows, ori_cols), crop_size, stride_rate, device
            )
